<script>
	...
	let tableData = writable(data.linearTickers);

	function change() {
		tableData.update((currentData) => {
			return currentData === data.linearTickers ? data.spotTickers : data.linearTickers;
		});
	}

	let currentTickers = writable(data.linearTickers);

	function changeCategory(category) {
		currentTickers.set(data[category]);
	}
</script>
		...
		<div class="flex space-x-5">
			...
			<div class="flex flex-col">
				<div class="flex justify-center items-center space-x-8">
					<button on:click={() => changeCategory('spotTickers')}>spot</button>
					<button on:click={() => changeCategory('linearTickers')}>Linear</button>
					<!-- <button on:click={() => changeCategory('inverseTickers')}>Inverse</button> -->
				</div>
				<Table tableData={$currentTickers} />
			</div>
		</div>
	</div>
</div>
